...
def get_config_dir_path():
    path = os.environ.get('XGD_CONFIG_HOME')
    if path is None or len(path) == 0:
        user_home = os.environ.get('HOME')
        if user_home is None or len(user_home) == 0:
            sys.exit(_("Invalid user home directory, is system sane?"))
...
def get_config_file_path():
    ...
    if not os.path.isfile(path):
        if not create_dummy_config(path):
            sys.exit(
                _("Failed to create config file {path}").format(path=path))
    return path
...
class Config:
    ...
    def dump_config(self):
        ...
        print(
            f"\t{ConfigKeys.DISCORD_ADMIN_ROLES.value} = {self.discord_admin_roles}")
        print(
            f"\t{ConfigKeys.DISCORD_MOTD_CHANNEL.value} = {self.discord_motd_channel}")
        print(
            f"\t{ConfigKeys.DISCORD_RAID_CHANNEL.value} = {self.discord_raid_channel}")
